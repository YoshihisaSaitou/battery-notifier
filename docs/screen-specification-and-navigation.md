...
- 5～100%のスライダーと現在値を表示する。左に円形の減少ボタン、中央に可変幅のスライダー、右に円形の増加ボタンを同じ操作行へ配置する。
- スライダー直下には数値目盛りを表示しない。設定中の値は既存の現在値表示で確認できるようにする。
- 減少・増加ボタンは1回につき1%変更し、5%では減少、100%では増加を無効化する。各ボタンは48dp以上のタップ領域と日英のTalkBackラベルを持つ。
- スライダーも5～100%の96個の離散値を1%刻みで操作・読み上げできるようにする。離散値を示すための目盛り点や数値目盛りは表示しなくてよい。
- 保存は明示ボタン方式とし、保存値と異なる下書きは未保存と表示する。連続ドラッグ中にData Layerへ送信せず、保存済みであると誤表示しない。
...
- ボタン付近に、電池状態は通常自動で同期されることと、自動同期されない場合または最新状態へ更新したい場合に`今すぐ同期 / Sync now`を使用することを常時表示する。
- ユーザーが同期を開始したら、結果確定までボタンを無効化し、ボタン内の進捗表示と`同期中… / Syncing…`、補助文の`スマートフォンの電池状態を同期しています。 / Syncing the phone battery state.`を同時に表示する。高速完了時も同期中表示を500ms以上保持し、処理中にIdle文言へ戻さない。
