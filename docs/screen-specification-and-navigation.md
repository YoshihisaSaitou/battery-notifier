...
- 画面を構成するスクロール領域の末尾に、ウォッチフェイスの電池残量へ`!`が付く条件（最後の更新から5分超または時刻ずれ）、表示値が古い可能性、および接続・時刻確認後に`今すぐ同期 / Sync now`を使う案内を日英で常時表示する。
...
W-001の`ScalingLazyColumn`末尾には、ウォッチフェイスの電池残量へ`!`が付く条件（最後の更新から5分超または時刻ずれ）、表示値が古い可能性、および接続・時刻確認後に`同期を再試行 / Retry sync`を使う案内を日英で常時表示する。現在のFreshnessやデータ有無にかかわらず説明へスクロール到達できるようにする。
