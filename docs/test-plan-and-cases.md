...
| TC-E094 | Pixel 10 Pro Foldの外側/内側/分割画面でMobile通知しきい値欄を日英・最大フォント・TalkBackで表示し、5/20/100から左右ボタンとスライダーを操作 | 減少ボタン・スライダー・増加ボタンが同じ行に並び、スライダー直下の`5 / 50 / 100`が対応位置に表示される。各ボタンは48dp以上で意味順に読み上げられ、1%刻みで範囲外へ進まず、保存前の永続化・同期は0件、保存後は最終値だけが反映される |
